#ifndef R_PBR_PBCORE_H
#define R_PBR_PBCORE_H
...
// Defines a light and get locations from environment PBR shader
...
void
DrawModelPBR(Model model, MaterialPBR mat, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale);
...
// Load an environment cubemap, irradiance, prefilter and PBR scene
Environment
LoadEnvironment(const char *filename, int cubemapSize, int irradianceSize, int prefilterSize, int brdfSize) {
    Environment env = {0};
    ...
    SetShaderValue(irradianceShader, GetShaderLocation(irradianceShader, "environmentMap"), (int[1]) {0},
                   SHADER_UNIFORM_INT);

    // Set up prefilter shader constant values
    SetShaderValue(prefilterShader, GetShaderLocation(prefilterShader, "environmentMap"), (int[1]) {0},
                   SHADER_UNIFORM_INT);
    ...
    for (unsigned int i = 0; i < 6; i++) {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, env.cubemapId,
                               0);
    ...
    for (unsigned int i = 0; i < 6; i++)
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, irradianceSize, irradianceSize, 0, GL_RGB,
                     GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    for (unsigned int i = 0; i < 6; i++)
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGB16F, prefilterSize, prefilterSize, 0, GL_RGB,
                     GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
    ...
    Matrix defaultProjection = MatrixPerspective(60.0, (double) GetScreenWidth() / (double) GetScreenHeight(), 0.01,
                                                 1000.0);
...
void UpdateLightValues(Environment env, Light light) {
    ...
    float diff[4] = {(float) light.color.r / (float) 255, (float) light.color.g / (float) 255,
...
DrawModelPBR(Model model, MaterialPBR mat, Vector3 position, Vector3 rotationAxis, float rotationAngle, Vector3 scale) {
    ...
    float shaderAlbedo[3] = {(float) mat.albedo.color.r / (float) 255, (float) mat.albedo.color.g / (float) 255,
                             (float) mat.albedo.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.albedo.colorLoc, shaderAlbedo, 3);
    float shaderNormals[3] = {(float) mat.normals.color.r / (float) 255, (float) mat.normals.color.g / (float) 255,
                              (float) mat.normals.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.normals.colorLoc, shaderNormals, 3);
    float shaderMetalness[3] = {(float) mat.metalness.color.r / (float) 255,
                                (float) mat.metalness.color.g / (float) 255,
                                (float) mat.metalness.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.metalness.colorLoc, shaderMetalness, 3);
    float shaderRoughness[3] = {1.0f - (float) mat.roughness.color.r / (float) 255,
                                1.0f - (float) mat.roughness.color.g / (float) 255,
                                1.0f - (float) mat.roughness.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.roughness.colorLoc, shaderRoughness, 3);
    float shaderAo[3] = {(float) mat.ao.color.r / (float) 255, (float) mat.ao.color.g / (float) 255,
                         (float) mat.ao.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.ao.colorLoc, shaderAo, 3);
    float shaderEmission[3] = {(float) mat.emission.color.r / (float) 255, (float) mat.emission.color.g / (float) 255,
                               (float) mat.emission.color.b / (float) 255};
    SetShaderValue(mat.env.pbrShader, mat.emission.colorLoc, shaderEmission, 3);
    float shaderHeight[3] = {(float) mat.height.color.r / (float) 255, (float) mat.height.color.g / (float) 255,
                             (float) mat.height.color.b / (float) 255};
    ...
    SetShaderValue(mat.env.pbrShader, mat.metalness.useBitmapLoc, (int[1]) {mat.metalness.useBitmap},
                   SHADER_UNIFORM_INT);
    SetShaderValue(mat.env.pbrShader, mat.roughness.useBitmapLoc, (int[1]) {mat.roughness.useBitmap},
                   SHADER_UNIFORM_INT);
